...
// -----------------------------------------
// NORMALIZE ALL PROGRAMS INTO ONE LIST
// Each course becomes:
// { code, credits, area, program }
// -----------------------------------------

function extractCourses(program: any) {
  const programName = program.program_name || "Unknown Program";

  return Object.entries(program.course_meta).map(([code, meta]: any) => ({
    code,
...
const COURSES = ALL_PROGRAMS.flatMap(extractCourses);
  ...
  return (
    <div className="card bg-gray-900 text-white p-6 rounded-lg shadow-lg mt-6">
      ...
      <form onSubmit={handleSearch} className="flex gap-3 mb-4">
        <input
          type="text"
          placeholder="Try: algorithms, cybersecurity, AI, database, calculus…"
          value={query}
      ...
      {results.length > 0 ? (
        <>
          ...
          <ul className="space-y-2">
            {results.map((c, i) => (
              ...
              >
                <strong className="text-blue-300">{c.code}</strong>
        ...
        query.length > 0 && (
          <p className="text-red-400">No matches found for “{query}”.</p>
        )
      )}
    </div>
  );
